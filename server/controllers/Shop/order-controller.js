...
export const processPeyment = async (req, res) => {
    try {
...
export const createOrder = async (req, res) => {
    try {
        const { user, products, addressInfo, totalAmount, paymentMethod, razorpay_order_id, razorpay_payment_id, razorpay_signature } = req.body;
        ...
        if (paymentMethod === 'COD') {
            ...
            return res.status(201).json({ success: true, message: "COD Order Placed Successfully!"});
        } else if (paymentMethod === 'Razorpay') {

            if (!user || !products || !addressInfo || !totalAmount || !paymentMethod || !razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
                return res.status(403).json({
                    success: false,
                    message: "Invalid data provided."
                })
...
export const fetchAllOrderByUserId = async (req, res) => {
    try {
...
export const fetchOrderDetailsById = async (req, res) => {
    try {
