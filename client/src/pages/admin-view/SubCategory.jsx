...
import { addSubCategory, fetchAllSubCategory, updateSubCategory } from '@/store/subcategory-slice';
...
const AdminSubCategory = () => {
  ...
  const { isLoading, subCategoryList } = useSelector(state => state.adminSubCategory);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(action).then(({ payload }) => {
      toast({ description: payload.message, variant: payload.success ? 'default' : 'destructive' });
      if (payload.success) {
        resetForm();
        dispatch(fetchAllSubCategory());
      }
  ...
  return (
    <>
      <div className='mt-14 mb-5 w-full flex justify-end'>
        <Button onClick={() => setOpenCreateSubCategory(true)}>Add New SubCategory<PlusCircleIcon/></Button>
      </div>
        ...
        <Table className='text-[1rem]'>
          ...
          <TableBody>
            {subCategoryList.length > 0 ? (
              ...
              <TableRow className='font-semibold'><TableCell>No Sub Category</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Sheet open={openCreateSubCategory} onOpenChange={resetForm}>
        <SheetContent side='right' className='overflow-auto'>
          <SheetHeader>
            <SheetTitle>{editValueId ? 'Edit SubCategory' : 'Add New SubCategory'}</SheetTitle>
          </SheetHeader>
          <form onSubmit={handleSubmit} className='space-y-3 mt-5'>
            <div className='space-y-2'>
              <Label htmlFor='category'>Category</Label>
              <Select id='category' onValueChange={value => setFormData({ ...formData, category: value })} value={formData.category}>
                <SelectTrigger>
            ...
            <div className='space-y-2'>
              ...
              <Input type='text' id='name' name='name' placeholder='Enter the name' value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
              {errors.name && <p className='text-red-600'>{errors.name[0]}</p>}
            </div>
            <Button className='w-full'>{editValueId ? 'Update' : 'Save'}</Button>
          </form>
        </SheetContent>
      </Sheet>
    </>
