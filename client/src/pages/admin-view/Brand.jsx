...
const AdminBrand = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const action = editValueId ? updateBrand({ formData, id: editValueId }) : addBrand(formData);
  ...
  <>
    <div className='mt-14 mb-5 w-full flex justify-end'>
      <Button onClick={() => setOpenCreateBrand(true)}>Add New Brand <PlusCircleIcon /> </Button>
    </div>
    {!isLoading ? (<Table className='text-[1rem]'>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {brandList.length > 0 ? brandList.map((item, index) => {
          return (<BrandItem srNo={index} key={item.name} brand={item} setFormData={setFormData} setOpenCreateBrand={setOpenCreateBrand} setEditValueId={setEditValueId} />)
        }) : (<TableRow className='font-semibold'><TableCell>No Brand</TableCell></TableRow>)}

      </TableBody>
    </Table>) : Array(10).fill(null).map((_, index) => {
    ...
    <Sheet open={openCreateBrand} onOpenChange={() => {
      resetForm();
    }
      ...
      <SheetContent side="right" className="overflow-auto" aria-describedby={undefined}>
        <SheetHeader>
          <SheetTitle>{editValueId ? 'Edit Brand' : 'Add New Brand'}</SheetTitle>
        </SheetHeader>
        <form onSubmit={handleSubmit}>
          <div className='space-y-3 mt-5'>
            <div className='space-y-2'>
              <Label htmlFor={'name'}>Brand Name</Label>
              <Input type="text" name="name" onChange={handleChange} placeholder='Enter the brand name' value={formData.name} />
