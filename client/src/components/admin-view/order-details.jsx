...
import { fetchAllOrder, fetchOrderDetailsById, updateOrderStatus } from '@/store/admin-order-slice'
...
const AdminOrderDetailsView = ({ orderDetails }) => {
    ...
    function handleUpdateStatus(e) {
        e.preventDefault();
        dispatch(updateOrderStatus({orderId:orderDetails._id,formData})).then(data=>{
    ...
    return <DialogContent aria-describedby={undefined} className='sm:max-w-[600px] h-screen overflow-auto'>
        <div className='grid gap-6'>
            <div className='grid gap-2'>
                ...
                <div className='flex mt-2 items-center justify-between'>
                    ...
                    <Label><Badge className={`${orderDetails?.paymentStatus == 'confirmed' ? 'bg-green-600' : 'bg-black'}`} >{orderDetails?.paymentStatus}</Badge></Label>
                </div>
                <div className='flex mt-2 items-center justify-between'>
                    <p className='font-medium'>Order Status</p>
                    <Label><Badge className={`${orderDetails?.orderStatus == 'confirmed' ? 'bg-green-600' : 'bg-black'}`} >{orderDetails?.orderStatus}</Badge></Label>
                </div>
                <div className='flex mt-2 items-center justify-between'>
