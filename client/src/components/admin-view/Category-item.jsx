...
const CategoryItems = ({ srNo, category, setOpenCreateCategory, setFormData, setEditedIdAndOldLogo }) => {
        ...
        <TableRow>
            ...
            <TableCell ><div>
                <div className="relative w-[50px] h-[50px] ">
                    <img src={`http://localhost:8000/${category.logo}`} alt={category?.name} className="h-full w-full  object-cover rounded-lg" />
                </div>
            </div></TableCell>
            <TableCell className='flex gap-4' > <Button onClick={() => {
                ...
                <Button onClick={() => deleteHandler(category._id)} className={'h-8 px-3 bg-red-600 py-1 text-sm'}>Delete<TrashIcon /></Button></TableCell>
        </TableRow>
    )
}
