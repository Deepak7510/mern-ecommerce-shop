...
function ShoppingProductItem({ product,handleDelete }) {
  ...
  return (
    <Card className="shadow-lg relative h-fit hover:shadow-xl transition-shadow duration-300">
          {handleDelete?<div onClick={()=>handleDelete(product._id)} className='cursor-pointer absolute top-3 right-3'><XIcon /></div>:null}
        ...
        <CardContent className="p-2 lg:p-3">
          <div className='absolute top-3 left-3'>
            {product.stock === 0 ?
              ...
              product.stock <= 10 ?
                <Badge className={'bg-red-600'}>left only {product.stock}</Badge> : null
            }
          </div>
          <img
          ...
          <div className="text-gray-600 text-sm lg:text-base">{productTitle}</div>
          <div className="flex items-center gap-2">
            <span className="text-md lg:text-lg font-bold">₹{product.salePrice}</span>
            <span className="text-gray-500 line-through text-sm lg:text:md">₹{product.price}</span>
            <span>{discount}% off</span>
          </div>
          <div className="text-gray-600 text-xs lg:text-sm">Size-{product.size}</div>
        </CardContent>
      </a>
    </Card>
  );
