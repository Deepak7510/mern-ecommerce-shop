...
function ProductFilter({ filter, handleFilter }) {
    ...
    const { isLoading: isLaoadigBrand, brandList } = useSelector(state => state.adminBrand);
            ...
            <div className='flex flex-col h-full '>
                        ...
                        {isLoading ? (Array(5).fill(null).map((_, index) => (
                            ...
                            categoryList && categoryList.length > 0 ?
                                categoryList.map(item => {
                                    return <Label className='flex items-center gap-2 font-medium' key={item._id}>
                                        <Checkbox
                                            checked={
                                                filter
                                                &&
                                            ...
                                            onCheckedChange={() => handleFilter('category', item._id)} />{item.name}
                                    </Label>
                                }) : null
                        }
                    </div>
                ...
                <div className='space-y-2 p-4'>
                    ...
                    <div className='space-y-4 grid'>
                        {isLaoadigBrand ? (Array(5).fill(null).map((_, index) => (
                            <div key={index} className="flex flex-col space-y-3">
                                <div className="space-y-2">
                                    <Skeleton className="h-4 w-full]" />
                                </div>
                            ...
                            brandList && brandList.length > 0 ? brandList.map(item => {
                                return <Label className='flex items-center gap-2 font-medium' key={item._id}>
                                    <Checkbox
                                        ...
                                        onCheckedChange={() => handleFilter('brand', item._id)} />{item.name}
                                </Label>
                            }) : null}
                    </div>
                </div>
