...
// Add New Category
export const addNewCategory = createAsyncThunk('/category/add', async (formData) => {
    try {
        const response = await axios.post(
            'http://localhost:8000/api/admin/category/add', 
            formData, 
            {
...
export const updateCategory = createAsyncThunk('/category/update', async ({ newFormData, id }) => {
    try {
        const response = await axios.put(
            `http://localhost:8000/api/admin/category/update/${id}`, 
            newFormData, 
            {
                headers: { "Content-Type": 'multipart/form-data' },
                withCredentials: true
...
export const deleteCategory = createAsyncThunk('/category/delete', async (id) => {
    try {
        const response = await axios.delete(
            `http://localhost:8000/api/admin/category/delete/${id}`, 
            { withCredentials: true }
        );
        return response.data;
    } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchAllCategory.fulfilled, (state, action) => {
                state.isLoading = false;
                state.categoryList = action.payload.success ? action.payload.categoryList : [];
            })
