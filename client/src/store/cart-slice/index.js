  ...
  async ({ userId, productId, quantity, size }) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URI}/api/shopping/cart/add`,
  ...
  async ({ userId }) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URI}/api/shopping/cart/get/${userId}`
      );
      return response.data;
    } catch (error) {
      // console.error(error?.response?.data)
  ...
  async ({ userId, productId, quantity, size }) => {
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_BACKEND_URI}/api/shopping/cart/update-cart`,
        { userId, productId, quantity, size }
      );
      return response.data;
    } catch (error) {
